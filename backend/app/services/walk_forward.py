...
def _split_date_range(start: date, end: date, n_folds: int, train_pct: float):
    ...
    if train_days < 20 or oos_days < 10:
        raise ValueError("train_pct produces windows that are too short. Use fewer folds or a wider date range.")
...
async def run_walk_forward(
    ...
    for fold_idx, (is_start, is_end, oos_start, oos_end) in enumerate(splits):
        ...
        is_config = config.model_copy(update={
        ...
        oos_config = config.model_copy(update={
