...
export default function WalkForwardPage({
  ...
  const { job, run, isRunning } = useJobRunner<WalkForwardResult>();
  ...
  const handleRun = async () => {
    ...
    try {
      const res = await run(
        () => api.runWalkForward(result.config, nFolds, trainPct),
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6">
        ...
        >
          <div>
            ...
            <p className="text-[10px] text-text-muted mt-1">
              {nFolds} folds × {Math.round(100 / nFolds)}% of the date range
              each
            </p>
          </div>
          <div>
          ...
          >
            {isRunning ? (
              <>
                ...
                {job?.status === "queued"
                  ? "Queued…"
                  : `${job?.progress_current ?? 0} / ${job?.progress_total || nFolds} folds`}
              </>
            ) : (
              "Run Analysis"
            )}
          ...
          >
            ...
            Each fold divides time into in-sample (IS) and out-of-sample (OOS)
            windows. The strategy runs with <em>fixed parameters</em> on each
            OOS slice. Stitching the OOS slices gives a continuous deployable
            equity curve.{" "}
            <span className="text-accent-green">Sharpe Efficiency</span> = avg
            OOS Sharpe ÷ avg IS Sharpe. Values near 1.0 indicate robustness.
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-5">
          {!wfa && !isRunning && (
            <div
              className="h-72 flex items-center justify-center rounded-md"
              style={{
                background: "var(--color-bg-card)",
                border: "1px solid var(--color-border)",
              }}
          ...
          {isRunning && (
            <div
              className="h-72 flex flex-col items-center justify-center gap-3 rounded-md"
              style={{
                background: "var(--color-bg-card)",
                border: "1px solid var(--color-border)",
              }}
            >
              ...
              <p className="text-sm text-text-secondary">
                {job?.status === "queued"
                  ...
                  : job?.progress_message || `Running ${nFolds} fold pairs…`}
              </p>
                ...
                {[
                  {
                    ...
                    content: <EfficiencyBadge value={wfa.sharpe_efficiency} />,
                  },
                  {
                    label: "OOS Total Return",
                    content: (
                      <span
                        className={`font-mono font-bold text-lg ${(wfa.oos_metrics.total_return_pct ?? 0) >= 0 ? "text-accent-green" : "text-accent-red"}`}
                      >
                        {formatPercent(wfa.oos_metrics.total_return_pct ?? 0)}
                      </span>
                    ),
                  },
                  {
                    label: "OOS Sharpe",
                    content: (
                      <span className="font-mono font-bold text-lg text-accent-blue">
                        {formatRatio(wfa.oos_metrics.sharpe_ratio ?? 0)}
                      </span>
                    ),
                  },
                  {
                    ...
                    content: (
                      <span className="font-mono font-bold text-lg text-accent-red">
                        {formatPercent(wfa.oos_metrics.max_drawdown_pct ?? 0)}
                      </span>
                    ),
                  },
                ].map(({ label, content }) => (
              ...
              >
                ...
                >
                  ...
                  <p className="text-[10px] text-text-muted mt-0.5">
                    Grey = in-sample (full period, given params) · Teal =
                  ...
                  <thead>
                    ...
                    >
                      {[
                        ...
                        <th
                          key={h}
                          className={`section-label py-2.5 px-3 font-normal ${["IS Return", "OOS Return", "IS Sharpe", "OOS Sharpe", "OOS MaxDD"].includes(h) ? "text-right" : "text-left"}`}
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {wfa.folds.map((f) => (
                      ...
                      >
                        ...
                        <td className="py-2 px-3 text-text-muted text-[10px]">
                          {f.is_start.slice(0, 7)} → {f.is_end.slice(0, 7)}
                        </td>
                        <td className="py-2 px-3 text-text-muted text-[10px]">
                          {f.oos_start.slice(0, 7)} → {f.oos_end.slice(0, 7)}
                        </td>
                        <td
                          className={`py-2 px-3 text-right tabular-nums ${(f.is_return ?? 0) >= 0 ? "text-accent-green" : "text-accent-red"}`}
                        >
                          {f.is_return != null
                            ? formatPercent(f.is_return)
                            : "—"}
                        </td>
                        <td className="py-2 px-3 text-right tabular-nums text-text-primary">
                          {f.is_sharpe?.toFixed(3) ?? "—"}
                        </td>
                        <td
                          className={`py-2 px-3 text-right tabular-nums font-semibold ${(f.oos_return ?? 0) >= 0 ? "text-accent-green" : "text-accent-red"}`}
                        >
                          {f.oos_return != null
                            ? formatPercent(f.oos_return)
                            : "—"}
                        </td>
                        <td className="py-2 px-3 text-right tabular-nums text-accent-blue font-semibold">
                          {f.oos_sharpe?.toFixed(3) ?? "—"}
                        </td>
                        <td className="py-2 px-3 text-right tabular-nums text-accent-red">
                          {f.oos_max_dd != null
                            ? formatPercent(f.oos_max_dd)
                            : "—"}
                        </td>
